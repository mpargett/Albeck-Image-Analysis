function [tracksFinal,kalmanInfoLink,errFlag,svo] = trackCloseGapsKalmanSparse_mp(...
    movieInfo,costMatrices,gapCloseParam,kalmanFunctions,probDim,...
    saveResults,verbose)
%TRACKCLOSEGAPSKALMANSPARSE (1) links features between frames, possibly
%   using the Kalman Filter for motion propagation and (2) closes gaps,
...
if selfAdaptive
    ...
    [tmp,dummy,kalmanInfoLink,dummy,linkingCosts] = linkFeaturesKalmanSparse_mp(...
    ...
    [dummy,dummy,kalmanInfoLink,dummy,linkingCosts] = linkFeaturesKalmanSparse_mp(...
    ...
    [tracksFeatIndxLink,tracksCoordAmpLink,kalmanInfoLink,nnDistLinkedFeat,...
        dummy,errFlag] = linkFeaturesKalmanSparse_mp(movieInfo,costMatrices(1).funcName,...
...
%% Close gaps
    %If there are no gaps to close (no tracks that start after the first frame,
    %   or no tracks that end before the last frame), finalize and return
    if ~any(trackStartTime > 1) || ~any(trackEndTime < numFramesEff)
        if verbose;  	disp('No gaps to close!');      end
        %Convert matrix of tracks into structure
        tracksFinal = convertMat2Struct(tracksCoordAmpLink,tracksFeatIndxLink);
        %Run finalizing operations and any diagnostics, then return
        tracksFinal = sub_final(tracksFinal, emptyStart, diagnostics); return;
    end
    if verbose; disp(sprintf('Closing gaps (%d starts and %d ends) ...',...
            length(find(trackStartTime>1)),length(find(trackEndTime<numFramesEff)))); %#ok<DSPS>
        progressText(0,'Gap closing');      %initialize progress display
    end

%Calculate the cost matrix, which already includes birth/death costs
% -- USER DEFINED FUNCTION -- %
%   Get cost function from parameters
cfun = str2func(costMatrices(2).funcName);
%Call for cost matrix from user-specified cost function
[costMat,nonlinkMarker,indxMerge,numMerge,indxSplit,numSplit,errFlag,svo]...
    = cfun(tracksCoordAmpLink, tracksFeatIndxLink, trackStartTime,...
    trackEndTime, costMatrices(2).parameters, gapCloseParam,...
    kalmanInfoLink, nnDistLinkedFeat, probDim, movieInfo); %#ok<ASGLU>

    %If there are no possible links, finalize and return
...
%Link tracks based on cost, allowing for birth and death
%   Linear Assignement Problem sets one link per track
[link12,link21] = lap(costMat,nonlinkMarker);

%Get Gap-Closing indices (Preceeding tracks)
gc = find(link12(1:numTracksLink) <= numTracksLink);

%Copy each later track into its earlier link, starting at the end, to avoid
%perturbing indices needed later in the loop (tracks are ordered by start
%time)            
for s = gc(end:-1:1)'
    %   Get valid time range for track to append
    tv = trackStartTime(link12(s)):trackEndTime(link12(s));
    tv8 = (8*(tv(1)-1)+1):(8*tv(end));
    %   Append track info into previous track
    tracksFeatIndxLink(s, tv) = tracksFeatIndxLink(link12(s), tv);
    tracksCoordAmpLink(s, tv8) = tracksCoordAmpLink(link12(s), tv8);
    nnDistLinkedFeat(s, tv)   = nnDistLinkedFeat(link12(s), tv);
    %Interpolate coordinates between gap closures
    tv8 = bsxfun(@plus, 8*((trackEndTime(s):tv(1))-1)', 1:probDim);
    tracksCoordAmpLink(s, tv8) = reshape( interp1(tv8([1,end],1), ...
        reshape(full(tracksCoordAmpLink(s, tv8([1,end],:))), [probDim,2]), ...
        tv8(:,1)), [1,numel(tv8)]);    
    %   Update receiving track end time
    trackEndTime(s) = trackEndTime(link12(s));
end
%Remove tracks that were linked to preceding tracks
trackStartTime(link12(gc)) = [];   	tracksFeatIndxLink(link12(gc),:) = [];   
trackEndTime(link12(gc)) = [];      tracksCoordAmpLink(link12(gc),:) = [];
nnDistLinkedFeat(link12(gc),:) = [];
%Update number of tracks
numTracksLink = size(tracksFeatIndxLink,1); 

%Generate final compound tracks structure
a1 = ones(1,1,numTracksLink);  aN = nan(1,1,numTracksLink);
tracksFinal = struct('tracksFeatIndxCG',num2cell(tracksFeatIndxLink,2),...
	'tracksCoordAmpCG',num2cell(tracksCoordAmpLink,2),...
	'seqOfEvents', squeeze(num2cell([shiftdim(trackStartTime,-2),a1,a1,aN;...
                        shiftdim(trackEndTime,-2),2*a1,a1,aN], [1,2])) );
tfn = fieldnames(tracksFinal);  ntfn = numel(tfn); %Fieldnames and size


%% Reassess and assign Merge/Splits
if mergeSplit
    %Apply Merge/Split search radius multipier
    costMatrices(2).parameters.brownStdMult = ...
       costMatrices(2).parameters.brownStdMult .* gapCloseParam.msRadMult;
    costMatrices(2).parameters.linStdMult = ...
       costMatrices(2).parameters.linStdMult .* gapCloseParam.msRadMult;
    %Call Gap-Closing cost function again, for Merge/Splits on revised tracks
    [costMat,nonlinkMarker,indxMerge,numMerge,indxSplit,numSplit,errFlag,svo_ms]...
        = cfun(tracksCoordAmpLink, tracksFeatIndxLink, trackStartTime,...
        trackEndTime, costMatrices(2).parameters, gapCloseParam,...
        kalmanInfoLink, nnDistLinkedFeat, probDim, movieInfo);
    
    %Append relevant MS search region info to svo
    for sf = {'maxDisp','type','lvs','svs','poss','pose'};
        svo.ms.(sf{1}) = svo_ms.(sf{1});
    end
    
   %Perform new linear assigment with revised tracks
    %   Avoid running lap if no splits/merges were found (NaNs => Error)
    if numSplit + numMerge > 0
        [link12,link21] = lap(costMat,nonlinkMarker);
    end
    
    %Get indices of Merging tracks (filters birth/death)
    if numMerge < 1; mg = [];   else
        mg = find(  link12(1:numTracksLink) > numTracksLink & ...
            link12(1:numTracksLink) <= numTracksLink+numMerge   );
        indxMerge = indxMerge(link12(mg) - numTracksLink); %Update list
        numMerge = numel(indxMerge);    %Update count
    end
    %Get indices of Splitting tracks (filters birth/death)
    if numSplit < 1; sp = [];   else
        sp = find(  link21(1:numTracksLink) > numTracksLink & ...
            link21(1:numTracksLink) <= numTracksLink+numSplit   );
        indxSplit = indxSplit(link21(sp) - numTracksLink); %Update list
        numSplit = numel(indxSplit);    %Update count
    end

    %   Get joint list of Merge/Splits, with labels
    ms = [[mg;sp],ones(numMerge+numSplit,1)];
    %   Get Recipient/Parent Indices
    rp = [[indxMerge; indxSplit],ones(numMerge+numSplit,1)];
    %   Get flag for if a Merge (vs. Split)
    iss = [false(numMerge,1);true(numSplit,1)];
    
    %Perform each Merge/Split operation, one at a time
    for s = 1:size(ms,1)
        if ms(s,1) == rp(s,1) %IF in same compound track, no appending
            %Set Merging/Splitting reference index
            tracksFinal(ms(s,1)).seqOfEvents(2*ms(s,2)-iss(s),4) = rp(s,2);
            
        else    %IF in different compound tracks, append
            %   Get number of local tracks in Recipient/Parent
            ntl = size(tracksFinal(rp(s,1)).seqOfEvents, 1)/2;
            %Adjust Merging/Splitting Local Indices
            tracksFinal(ms(s,1)).seqOfEvents(:,3:4) = ...
                tracksFinal(ms(s,1)).seqOfEvents(:,3:4) + ntl;
            
            %Set Merging/Splitting reference index
            tracksFinal(ms(s,1)).seqOfEvents(2*ms(s,2)-iss(s),4) = rp(s,2);
            
            %Append Merging/Splitting info after Recipient/Parent info
            for ss = 1:ntfn;  tracksFinal(rp(s,1)).(tfn{ss}) = cat(1,...
                tracksFinal(rp(s,1)).(tfn{ss}), tracksFinal(ms(s,1)).(tfn{ss}));
            end
            
            %Update any dependent Recipient/Parent indices
            dp = rp(:,1) == ms(s,1);        %Indices of dependent RP
            if any(dp); rp(dp,:) = ones(nnz(dp),1)*[rp(s,1), ntl+rp(s,2)]; end
            %   And any Merging/Splitting indices (if both in same track)
            dp = ms(s+1:end,1) == ms(s,1);  %Indices of unrun dependent MS
            if any(dp); ms(dp,:) = ones(nnz(dp),1)*[rp(s,1), ntl+ms(s,2)]; end
        end
    end
    
    %Sort Sequence of Events by time (Only need for Recipients/Parents)
    rp = unique(rp(:,1));  %Final Recip/Parent index
    for s = rp'
        [~,si] = sort(tracksFinal(s).seqOfEvents(:,1)); %Get sorting index
        tracksFinal(s).seqOfEvents = tracksFinal(s).seqOfEvents(si,:);
    end
    
    %Remove now Merged/Split tracks
    tracksFinal(ms(:,1)) = [];    
end

%Remove extraneous columns from final compound track info
for s = 1:numel(tracksFinal)
    %Get start and end indices per track
    se = tracksFinal(s).seqOfEvents([1,end],1); 
    %Restrict info to range from start to end only
    tracksFinal(s).tracksFeatIndxCG = ...
        tracksFinal(s).tracksFeatIndxCG(:,se(1):se(2));
    tracksFinal(s).tracksCoordAmpCG = full( ...
        tracksFinal(s).tracksCoordAmpCG(:,(8*(se(1)-1)+1):(8*se(2))) );
    %For historical reasons, set to NaN tracksCoordAmpCG where the
    %   corresponding x-coordinate = 0?  Not currently implemented.
        x0 = repmat(permute(tracksFinal(s).tracksCoordAmpCG(:,1:8:end),...
            [1,3,2]) == 0,1,8,1);    sx = size(x0);
        tracksFinal(s).tracksCoordAmpCG(tracksFinal(s).tracksCoordAmpCG...
            == 0 & reshape(x0,[sx(1),prod(sx(2:3))])) = NaN;
end

%Display elapsed time, if doing displays
if verbose;     progressText(1,'Gap closing');  end

%Run finalizing operations and any diagnostics
tracksFinal = sub_final(tracksFinal, emptyStart, diagnostics);
%% Save results
% If this is desired, add to sub_final
% if isstruct(saveResults)
...
%% Finalizing and diagnostic subfunction
function tF = sub_final(tF, eS, dg)
%shift time if any of the initial frames are empty
for iTrack = 1 : length(tF)
    tF(iTrack).seqOfEvents(:,1) = ...
        tF(iTrack).seqOfEvents(:,1) + eS;
end

%If diagnostics are requested
if ~isempty(dg) && dg == 1
    %extract gap information from tracks
    gapInfo = findTrackGaps(tF);
...
% 
% %TEMP FOR TESTING
% fixi = @(xi,x)(xi - nnz(x<xi)).*all(xi ~= x);
% for s = 1:numel(indxSplit)
%     indxSplit(s) = fixi(indxSplit(s),link12(gc));
%     sp(s) = fixi(sp(s),link12(gc));
% end
% indxSplit = indxSplit(indxSplit>0); sp = sp(sp>0); 
% for s = 1:numel(indxMerge)
%     indxMerge(s) = fixi(indxMerge(s),link12(gc));
%     mg(s) = fixi(mg(s),link12(gc));
% end
% indxMerge = indxMerge(indxMerge>0); mg = mg(mg>0);
% %%%%

% % %STUPID CHECK FOR EQUIVALENCE
% tff = tracksFinal;
% for s = 1:numel(tft)
%     for sf = 1:ntfn
%         ise(s,sf) = isequal( tft(s).(tfn{sf})(~isnan(tft(s).(tfn{sf}))), ...
%             tff(s).(tfn{sf})(~isnan(tff(s).(tfn{sf}))) );
%     end
% end
% all(ise(:))

% 
% sst = zeros(ntfn,1);  ssf = sst;
% for s = 1:numel(tft)
%     sst(s,1:3) = structfun(@(x)nansum(nansum(x)), tft(s));
%     ssf(s,1:3) = structfun(@(x)nansum(nansum(x)), tff(s));
% end
