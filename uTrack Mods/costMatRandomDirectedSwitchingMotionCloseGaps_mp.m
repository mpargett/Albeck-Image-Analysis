function [costMat,nonlinkMarker,indxMerge,numMerge,indxSplit,numSplit,...
    errFlag,svo] = costMatRandomDirectedSwitchingMotionCloseGaps_mp(trackedFeatInfo,...
...
AmpWin = gapCloseParam.ampgap + (0:gapCloseParam.ampwin-1);
...
%Add measurement variance to state noise for search region estimates
for s = 1:numTracks
    noiseStd(s) = sqrt(noiseStd(s).^2 + sum(movieInfo(trackEndTime(s)). ...
        allCoord(trackedFeatIndx(s,trackEndTime(s)),2:2:end).^2));
end
...
%MP: Pass search vectors to output for diagnostics
pf = @(timeidx)cell2mat(arrayfun(@(ti,tv)kalmanFilterInfo(tv)...
    .stateVec(trackedFeatIndx(ti,tv),1:probDim), ...
    (1:numTracks)', timeidx, 'Un', 0));
svo = struct('lvs',longVecSAll, 'lve',longVecEAll,...
    'svs',shortVecSAll, 'sve',shortVecEAll,...
    'svs3',shortVecS3DAll, 'sve3',shortVecE3DAll,...
    'lvsMS',longVecSAllMS, 'lveMS',longVecEAllMS,...
    'svsMS',shortVecSAllMS, 'sveMS',shortVecEAllMS,...
    'svsMS3',shortVecS3DAllMS, 'sveMS3',shortVecE3DAllMS,...
    'lrvs',longRedVecSAll, 'lrve',longRedVecEAll,...
    'lrvsMS',longRedVecSAllMS, 'lrveMS',longRedVecEAllMS, ...
    'poss',[pf(trackStartTime),trackStartTime], ...
    'pose',[pf(trackEndTime),trackEndTime], 'trkhist', trackedFeatInfo(:,...
        reshape([1:8:8*numFrames;2:8:8*numFrames],1,2*numFrames)), ...
    'type', trackType, 'maxDisp',maxDispAllowed);
    ...
    if mergeSplit == 1 || mergeSplit == 2
        ...
        for endTime = 1 : numFrames-1
            ...
            for iPair = 1 : numPairs
                ...
                %last 5 points
                indxBefore = 8*(endTime-1)+4 - 8*(AmpWin);
                ...
                indxAfter = 8*endTime+4 + 8*(AmpWin);
    ...
    if mergeSplit == 1 || mergeSplit == 3
        ...
        for startTime = 2 : numFrames
            ...
            for iPair = 1 : numPairs
                ...
                %the first 5 points
                indxAfter = 8*startTime+4 + 8*(AmpWin);
                indxAfter = indxAfter(indxAfter < 8*numFrames);
                ...
                indxBefore = 8*(startTime-1)+4 - 8*(AmpWin);
                indxBefore = indxBefore(indxBefore > 1);
