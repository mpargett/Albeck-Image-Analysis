function [costMat,propagationScheme,kalmanFilterInfoFrame2,nonlinkMarker,...
    errFlag] = costMatRandomDirectedSwitchingMotionLink_mp(movieInfo,kalmanFilterInfoFrame1,...
...
kalmanStd = sqrt(probDim * abs(squeeze(kalmanFilterInfoFrame1.noiseVar(1,1,:))) ...
    + sum(movieInfo(1).allCoord(:,2:2:end).^2,2));
...
%MP: Pack searchRadius into output for diagnostics
kalmanFilterInfoFrame2.sr = searchRadius(:,1);
