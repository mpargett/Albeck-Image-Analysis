...
p.code = ['\\albecklab.mcb.ucdavis.edu\Code\Image Analysis\'];
...
%                       processing is run
p.bioformats = '\\albecklab.mcb.ucdavis.edu\bfmatlab\';

%Declare proper path for uTrack
% p.utrack = '';    %Default sets no new path
p.utrack = '\\albecklab.mcb.ucdavis.edu\u-track_2.1.0\software\';
...
%   expressions
pth = path;
p.bad = regexpi(pth, '[^;]*\\Code[^;]*;', 'match');
p.bad = [p.bad,regexpi(pth, '[^;]*\\u-track[^;]*;', 'match')];
end
